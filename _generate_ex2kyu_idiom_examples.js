...
const INPUT_FILE = path.join(__dirname, '単語リスト', '英検2級単熟語EX_熟語編_raw.json');
const OUTPUT_FILE = path.join(__dirname, 'public', 'wordlist_ex2kyu_idiom.json');
...
async function generateBatch(words) {
  ...
  const prompt = `以下の英熟語リストに対して、英検2級レベルの文法と構文を使った英語例文と日本語訳を生成してください。

ルール:
- 例文は1文で、15語以内
- 英検2級で出そうな自然な文脈（社会、科学、日常生活、環境、文化、教育など）
- 熟語の意味がわかる文脈で使用すること
- 各熟語の ~ などの記号部分は、実際の単語（名詞や代名詞など）に置き換えて自然な例文にすること
