...
// Excel読み込み
const wb = XLSX.readFile('D:\\Files\\(_英単語テスト)\\超基礎_ターゲット1400にあって1900にない単語.xlsx');
const ws = wb.Sheets[wb.SheetNames[0]];
const rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });

const allWords = rows.filter(r => typeof r[0] === 'number' && r[1]).map(r => ({
  rank: r[0],
...
console.log('Sample:', allWords.slice(0, 5).map(w => `${w.rank}. ${w.word} → ${w.meaning}`).join('\n  '));

const OUTPUT_FILE = 'C:\\Users\\makoto\\Documents\\アプリ開発\\VocabularyBase\\public\\wordlist_target1400extra.json';
