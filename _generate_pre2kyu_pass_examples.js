...
const INPUT_FILE = path.join(__dirname, '単語リスト', '英検準2級パス単_raw.json');
const OUTPUT_FILE = path.join(__dirname, 'public', 'wordlist_pre2kyu_pass.json');
...
async function generateBatch(words) {
  ...
  const prompt = `以下の英単語リストに対して、英検準2級レベルの文法と構文を使った英語例文と日本語訳を生成してください。

ルール:
- 例文は1文で、13語以内
- 英検準2級で出そうな自然な文脈（学校生活、部活動、旅行、環境問題、健康、科学技術など）
- 単語の意味がわかる文脈で使用すること
- 文法・構文は英検準2級レベル（不定詞の応用、分詞、関係代名詞、接続詞、比較の応用など高1レベル）
- 日本語訳は自然な日本語で
