...
const INPUT_FILE = path.join(__dirname, '単語リスト', '英検4kyuパス単_raw.json');
const OUTPUT_FILE = path.join(__dirname, 'public', 'wordlist_4kyu_pass.json');
...
async function generateBatch(words) {
  ...
  const prompt = `以下の英単語リストに対して、英検4級レベルの文法と構文を使った英語例文と日本語訳を生成してください。

ルール:
- 例文は1文で、10語以内
- 英検4級で出そうな自然な文脈（学校生活、家族、友人、買い物、旅行、スポーツなど中学1〜2年生の日常）
- 単語の意味がわかる文脈で使用すること
- 文法は英検4級レベル（過去形、未来形、助動詞、不定詞to do、比較級、there is/areなど中1〜中2範囲）
- 日本語訳は自然な日本語で
