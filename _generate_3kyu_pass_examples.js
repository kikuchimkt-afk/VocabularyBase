...
const INPUT_FILE = path.join(__dirname, '単語リスト', '英検3級パス単_raw.json');
const OUTPUT_FILE = path.join(__dirname, 'public', 'wordlist_3kyu_pass.json');
...
async function generateBatch(words) {
  ...
  const prompt = `以下の英単語リストに対して、英検3級レベルの文法と構文を使った英語例文と日本語訳を生成してください。

ルール:
- 例文は1文で、12語以内
- 英検3級で出そうな自然な文脈（学校、家庭、趣味、旅行、友人関係、季節の行事など中学生の日常）
- 単語の意味がわかる文脈で使用すること
- 文法・構文は英検3級レベル（現在完了、受動態、不定詞、比較級、関係代名詞whatなど中学で学ぶ範囲）
- 日本語訳は自然な日本語で
