...
// Excel → JSON変換済みの整理済みリストを読み込む
// まずExcelから読み込むためのヘルパー（openpyxlで事前にJSONに変換済みの前提）
const INPUT_FILE = path.join(__dirname, '単語リスト', '英検2級単熟語EX_単語編_raw.json');
const OUTPUT_FILE = path.join(__dirname, 'public', 'wordlist_ex2kyu_tango.json');

if (!fs.existsSync(INPUT_FILE)) {
  console.error(`入力ファイルが見つかりません: ${INPUT_FILE}`);
  console.error('先にExcelからJSONに変換してください。');
  process.exit(1);
}
...
async function generateBatch(words) {
  ...
  const prompt = `以下の英単語リストに対して、英検2級レベルの文法と構文を使った英語例文と日本語訳を生成してください。

ルール:
- 例文は1文で、15語以内
- 英検2級で出そうな自然な文脈（社会、科学、日常生活、環境、文化、教育など）
- 単語の意味がわかる文脈で使用すること
- 文法・構文は英検2級レベル（関係代名詞、不定詞、動名詞、受動態、比較、接続詞など高校で学ぶ範囲）
- 日本語訳は自然な日本語で
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    for (let retry = 0; retry < MAX_RETRIES; retry++) {
      try {
        ...
        // 既存フォーマットに変換: rank, english, meanings, example, exampleJa
        const converted = generated.map(g => ({
          rank: g.rank,
          english: g.word,
          meanings: g.meaning,
          example: g.example,
          exampleJa: g.translation
        }));
